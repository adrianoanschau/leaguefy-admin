...
leaguefy.ajax = {}; // ajax loading
leaguefy.pages = {}; // shared logic for pages
  ...
  init: function () {
    function removeActiveClass() {
      let activeElements = document.querySelectorAll(
        'aside.main-sidebar ul > li > a.active',
      );
    ...
    let elements = document.querySelectorAll('aside.main-sidebar ul > li > a');
...
leaguefy.ajax = {
  ...
  init: function () {
    ...
    window.onpopstate = function (event) {
      ...
      leaguefy.ajax.navigate(document.location, preventPopState);
    };
      ...
      function (event) {
        if (event.target.matches('a[href], a[href] *')) {
          ...
          if (
            ...
            leaguefy.ajax.navigate(url, preventPopState);
            event.preventDefault();
          }
        }
      },
  ...
  navigate: function (url, preventPopState) {
    ...
    if (url != document.location.href) {
      ...
      leaguefy.menu.setActivePage(url);
    }
    ...
    axios(axios_obj)
      .then(function (response) {
        if (typeof result_function === 'function') {
          ...
          leaguefy.ajax.done(response);
        }
      })
      .catch(function (error) {
        leaguefy.ajax.error(error);
      })
      .then(function () {
        NProgress.done();
        if (
  ...
  error: function (error) {
    if (error.response) {
      ...
      leaguefy.ajax.done(error.response);
...
leaguefy.collectGarbage = function () {
